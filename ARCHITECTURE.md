...
<DATA_DIR>/
  ...
  designs/<designId>/      # (Hub) assets (logo/fond) de la bibliothèque de designs (§9bis).
...
- **`utils/design.js`** (design.F, §9bis) — runtime kiosque du design. `applyDesign(design)` pose les custom properties sur `<html>` : **itère sur `DESIGN_COLOR_KEYS`, jamais sur les clés reçues** (une clé hostile ne peut pas devenir une custom property), rayons/police depuis `RADIUS_PRESETS`/`FONT_PRESETS` de **core** (source partagée avec l'aperçu Hub `DesignPreview`). **Nettoie d'abord toutes les `MANAGED_VARS`** (sinon les couleurs d'un design précédent survivraient aux clés absentes du suivant) ; `applyDesign(null)` retire tout → le thème figé (`data-theme`) reprend la main. Appelé par `GuestPage` juste après `setAttribute('data-theme', …)`. Testé isolément (`test/design.test.js`).
- Variantes de layout (`design.layouts`) : `StartScreen` (`centered`/`cover`/`split`) et `ThankYouScreen` (`centered`/`cover`). L'image de fond n'est posée (inline `backgroundImage: url("…")`) que sur la variante `cover` ; l'URL vient de `resolveDesign` (donc d'un filename validé) — jamais une chaîne libre.
...
- `pages/DesignsPage.jsx` (route `/designs`, sous `RequireAuth` ; lien dans l'en-tête d'`EventsPage`) : bibliothèque de designs (§9bis). **Pas de router imbriqué** — un state `selectedId` bascule liste ↔ détail. Liste : « Mes designs » / « Templates » / (superuser) « Tous les designs » **groupés par `owner_email`** (champ que seul un superuser reçoit — cf. `listDesigns`). Cartes → Dupliquer / Renommer / Supprimer / Promouvoir / Rétrograder (`window.prompt`/`confirm`). Détail : `DesignEditor` + `VersionHistory`. Les gardes front (`canEdit`, `isSeed`) **miroitent** `canWrite`/`isSeedTemplate` du backend mais ne s'y substituent pas (le backend refuse en 403/409 de son côté).
- `components/designs/` (§9bis) :
  - `DesignEditor.jsx` — formulaire contrôlé des tokens. **Aucune saisie CSS libre** : couleurs par `<input type="color">` natif (+ champ hex texte pour l'alpha, l'input color ignorant le canal alpha → `toPickerValue()` ne lui montre que les 6 premiers hex), rayons/police par `<select>`, layouts par radios — les listes viennent des enums de core. La validation client utilise **`validateDesign` de core, la même fonction que le backend** (aucun écart de règle possible) ; « Enregistrer » est désactivé tant que la config est invalide, et un échec serveur restaure la config précédente.
  - `DesignPreview.jsx` — **maquettes statiques** des 4 écrans kiosque (accueil/prénom/enregistrement/merci), bascule de largeur 360/820/1280 (cadre à la largeur cible, réduit par `transform: scale()` dans un wrapper `overflow:hidden`). **Ne partage aucun code avec la borne** : dérive assumée, la borne d'essai reste la validation finale. `cssVarsFor()` construit les custom properties en **itérant sur `DESIGN_COLOR_KEYS`, jamais sur les clés reçues**, et lit les rayons/polices dans `RADIUS_PRESETS`/`FONT_PRESETS` de core (source unique partagée avec le futur runtime kiosque, design.F). Les valeurs ne finissent que dans des custom properties (`--*`), jamais dans une propriété CSS standard ni dans un `url()` — aucune valeur libre n'atteint le DOM (§11.28).
...
| `design.js` | Contrat d'un design (§9bis) : `DESIGN_COLOR_KEYS` (18), `DESIGN_RADIUS`, `DESIGN_FONTS`, `DESIGN_LAYOUTS`, `DESIGN_ASSET_SLOTS`, `DESIGN_KEYS` (whitelist **racine**), `DESIGN_VERSION`, `DESIGN_MAX_JSON` (16384), **`RADIUS_PRESETS` / `FONT_PRESETS`** (preset d'enum → valeur CSS réelle : source **unique** partagée par l'aperçu de l'éditeur Hub et le runtime kiosque — un test core garde l'invariant « chaque valeur d'enum a son preset », sinon le runtime resterait sans valeur CSS) + **`validateDesign(obj)`** → `{ok:true} \| {ok:false,error}`. **C'est LA barrière anti-injection CSS (§11.28)** : hex strict (`#rrggbb[aa]`), enums fermées, assets `<uuid>.png\|jpg\|webp` (jamais de chemin, jamais de SVG), taille ≤ 16 Ko. La validation itère sur **nos** listes de clés (jamais sur les clés reçues) et rejette toute clé inconnue. Fonction **pure, sans dépendance node** → importable par le backend Hub *et* les deux fronts (l'éditeur Hub valide avant envoi, le backend revalide toujours). | ✅ |
...
`event_meta.design` + `events/<id>/design/` (**snapshot, aucune référence conservée**) → bundle de pull
...
- **3 routes servent des fichiers de design** — aucune ne construit un chemin depuis l'URL sans confrontation préalable : `GET /api/designs/:id/assets/:filename` (Hub, vs `config.assets`), `GET /api/sync/events/:id/design/:filename` (Hub, vs `readdirSync` du dossier), `GET /api/event/design/:filename` (Borne, **publique**, vs `event_meta.design.assets`). Toute nouvelle route servant un fichier doit suivre le même patron (whitelist, jamais `join(dir, req.params.x)` nu).
