...
| `middleware/auth.js` | `makeAuthRouter` (login → JWT), `requireRole(role)` factory → `requireAdmin`/`requireTech`. **Phase C** — ni `admin_borne` ni `tech_borne` n'ont de compte nominatif : `{ pin }` comparé à `event_meta.tech_pin` (essayé en premier, rôle le plus élevé) puis `event_meta.admin_pin` de l'événement actif. **Fallback `TECH_PASSWORD`** (`{ password }` seul) — **refusé dès qu'un événement est actif** (`if (activeEvent || !password || …) → 401`) : c'est le seul recours en mode autonome ou juste après appairage, avant le premier pull ; une fois un `tech_pin` disponible il cesserait sinon d'être un secours pour devenir une porte dérobée permanente (a fortiori en preview, où `TECH_PASSWORD_PREVIEW` est partagé par toutes les bornes d'essai). ⚠️ **Corollaire** : un événement actif dépourvu de `tech_pin` en `event_meta` (pullé d'un Hub antérieur à Phase C) rend la console `/borne` inaccessible jusqu'au prochain pull — c'est le heartbeat qui répare. |
...
- **Phase B — deux consoles, deux sessions localStorage, deux auth distinctes** (`App.jsx` route sur `window.location.pathname`, toujours pas de react-router) :
  ...
  - **Phase C — écran d'onboarding pré-appairage** (`OnboardingScreen.jsx` + le hook `usePairingStatus`
    exporté par le même fichier) : tant que `/borne` n'est pas authentifié, `BornePage` interroge
    `GET /api/sync/pairing-status` (**sans auth**, toutes les 4 s) et rend `null` au tout premier
    chargement pour éviter un flash du mauvais écran. `hasToken === false` → `OnboardingScreen`
    (consigne « renseigner `BORNE_TOKEN` puis redémarrer » + journal `initLog`, aucun mot de passe
    demandé). Sinon `AdminLogin`, avec `mode = hasActiveEvent ? 'pin' : 'password'` — juste après
    l'appairage, aucun `tech_pin` n'a encore été pullé, seul `TECH_PASSWORD` peut ouvrir la console
    (et il n'est accepté par le serveur que dans cet état précis, cf. `middleware/auth.js`).
    ⚠️ **Deux cas limites explicitement gérés** : (1) en **mode 100 % autonome** (aucun token, aucun
    Hub prévu), `hasToken=false` afficherait l'`OnboardingScreen` (sans champ de mot de passe) — mais
    un **bouton d'échappement** « Pas de Hub prévu → `TECH_PASSWORD` » s'affiche dès que `autonome`
    (`!status.hubUrl && !status._error`) : `onSkip` pose `skipOnboarding=true` et `BornePage` bascule
    sur `AdminLogin` en `mode='password'`. Ce bouton **ne s'affiche pas** dans l'état dégradé `_error`
    (Hub configuré mais momentanément injoignable — `hubUrl` toujours `null` sur un échec réseau),
    pour ne pas faire croire à un mode autonome pendant une simple coupure. (2) si le tout premier
    appel à `pairing-status` échoue (réseau, 429 du rate-limiter avec plusieurs onglets ouverts),
    `usePairingStatus` **ne reste pas à `null`** : après le premier échec il pose un statut dégradé
    `{ hasToken:false, _error:true, … }` (bandeau « nouvelle tentative en cours » sur l'`OnboardingScreen`,
    jamais d'écran blanc), et le polling toutes les 4 s se rétablit seul dès la première réponse.
...
- *Borne* : JWT `{roles}` (admin_borne/tech_borne via PIN partagé, sans email depuis Phase C ;
  ...
  Fallback `TECH_PASSWORD` (`{ password }` seul) accepté **uniquement s'il n'y a aucun événement
  actif** (mode autonome / juste après appairage) — refusé sinon (cf. `middleware/auth.js`).
  ⚠️ L'auth wall preview (`general`) est câblée côté front sur `api.login` → `POST /api/admin/login`
  (`GuestPage.jsx`), route qui ne connaît plus que PIN et `TECH_PASSWORD` : le seul chemin serveur
  valide est `POST /api/preview/login` (`routes/sessions.js`).
