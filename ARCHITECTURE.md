...
- `provisionPreview` : **insère d'abord le box_token** (purge les `preview-auto` orphelins du
  ...
  (sinon 401 « Token borne invalide » → « aucun événement actif »). Plus de `TECH_PASSWORD` à
  fournir (retiré, PROJET.md §11.30) : un événement preview a toujours un `admin_pin`/`tech_pin`
  généré à sa création, disponible dès ce premier pull one-shot. Un token sans container est inoffensif.
...
Point d'entrée : `index.js` → `createApp(dataDir, cfg)` (ouvre le registre), puis **Phase C** `resolveJwtSecret()` (résout/génère `JWT_SECRET`, cf. `borneIdentity.js` ci-dessous) et **Phase B** `resolveBorneIdentity()` (lit/sème l'identité persistante), **avant** `validateConfig()` — qui ne rejette donc plus jamais un déploiement `.env.example-*` non modifié, `resolveJwtSecret()` ayant déjà remplacé toute valeur d'exemple par un secret généré. Au boot, deux chemins mutuellement exclusifs selon l'identité résolue : **borne physique** (`hubUrl && borneToken`) → pull one-shot `pullMyEvents()` puis `startHeartbeat()` (boucle `PULL_INTERVAL_MS`, pulls suivants + commandes automatiques) ; **preview / token=événement** (`hubUrl && boxToken`, `borneToken` absent) → pull one-shot `pullMyEvent()` inchangé, pulls suivants déclenchés manuellement depuis `/borne`. Ni l'un ni l'autre → pas encore appairée (aucun événement possible, cf. §11.29 PROJET.md : aucune route ne crée plus d'événement local) ; si elle est appairée plus tard via `POST /sync/onboarding/pair` (§ci-dessous), `startHeartbeat()` est démarré à ce moment-là (idempotent) plutôt qu'au boot.

### Modules transversaux

| Fichier | Responsabilité |
|---------|---------------|
| `config.js` | Env (port, jwtSecret, hubUrl, boxToken, previewMode, maxDataBytes…). **Phase B** — `borneToken` (`BORNE_TOKEN`, identité de borne physique, distincte de `boxToken`) et `pullIntervalMs` (`PULL_INTERVAL_MS`, déf. 300000 — ressuscite une variable déclarée dans `docker-compose.borne.yml` mais lue par aucun code avant Phase B). **Phase C** — `trustProxyHops` (`TRUST_PROXY_HOPS`, consommé par `app.set('trust proxy', …)` dans `index.js`) : nombre réel de reverse proxies devant le backend, **déduit de `PREVIEW_MODE`** (1 borne réelle → `borne-nginx` seul ; 2 preview → `edge-nginx` puis `preview-nginx`), surchargeable seulement si la topologie change (PROJET.md §11.31). ⚠️ Piège : `docker-compose.borne.yml` déclare `TRUST_PROXY_HOPS=${TRUST_PROXY_HOPS:-}`, donc la variable **existe toujours** dans le conteneur, éventuellement **vide** — un repli en `??` (nullish) ne se déclenche pas sur une chaîne vide et `parseInt('')` vaut `NaN`, ce qui **désactive silencieusement** `trust proxy` (`compileTrust` d'Express : `i < NaN` toujours faux) sans qu'`express-rate-limit` ne prévienne (ses validateurs ne testent que `=== true`/`=== false`). Même remarque pour toute nouvelle variable numérique câblée avec `:-` dans un compose. La résolution est **extraite en fonction pure exportée `resolveTrustProxyHops(env)`** (repli `||` + garde `Number.isInteger && >= 0`, testée dans `config.test.js` sans toucher `process.env` ni le cache du module ESM). |
| `registry.js` | Accès à `registry.sqlite` **local** (différent du Hub) : table `local_events` (1 seul `active=1` à la fois) + `push_state`. Helpers `getActiveEvent`, `setActiveEvent`, `updateEventStatus`. **Phase B** — `borne_settings` (`key TEXT PRIMARY KEY, value TEXT`) porte l'identité persistante de la machine (`borne_token`, `hub_url`) ; `getSetting`/`setSetting` (upsert `ON CONFLICT`). **Phase C** — deux clés s'y ajoutent : `jwt_secret` (écrite par `resolveJwtSecret()`, cf. `borneIdentity.js`) et `paired_at` (écrite **une seule fois**, au premier pull réussi de la vie de la borne, par `_setLastPull()` dans `sync/pull.js` — verrou persisté de `POST /sync/onboarding/pair`). `borne_settings` n'est donc plus seulement de l'*identité*, c'est le magasin des secrets et de l'état d'appairage de la machine — aucune donnée invité n'y entre (RGPD, §11), mais un `registry.sqlite` copié hors de la borne emporte désormais de quoi forger des JWT admin. ⚠️ `paired_at` n'est **pas backfillée** : une borne mise à niveau depuis une version antérieure (ou dont aucun pull n'a jamais abouti) démarre avec `paired_at` absente alors qu'elle porte déjà un token. Le verrou d'appairage ne s'y fie donc pas seul — il accepte **trois** signaux persistés en OU : `paired_at`, **au moins une ligne `local_events`** (preuve d'un pull abouti avant l'introduction de `paired_at`) et **`borne_settings.borne_token` non nul** (borne seedée par `BORNE_TOKEN` en `.env`, que `resolveBorneIdentity()` persiste au boot sans round-trip Hub). Cf. `routes/sync.js` ci-dessous et le piège « routes Borne sans auth » plus bas. ⚠️ Le token d'événement (`boxToken`, preview/legacy) n'est, lui, **jamais persisté** : il ne participe donc à aucun de ces trois signaux — asymétrie détaillée dans le piège en question. |
| `borneIdentity.js` | **Phase B** — `resolveBorneIdentity()` : à appeler après `openRegistry()`, avant tout appel Hub. Au premier démarrage sème `borne_settings` depuis `config.borneToken`/`config.hubUrl` (valeurs d'env) ; aux démarrages suivants la base **prime sur l'env** (mute `config.borneToken`/`config.hubUrl` en place) — permet de faire tourner le token sans redéployer. `hub_url` n'est seedé que si `config.borneToken` est renseigné : no-op total pour une preview (jamais de `BORNE_TOKEN`) et tant qu'aucun `BORNE_TOKEN` n'est configuré. **Phase C — `resolveJwtSecret()`** : même principe seed-puis-persistance (`jwt_secret` dans `borne_settings`) mais **sans** la garde sur `borneToken` — un JWT_SECRET est nécessaire dès le premier boot, avant même tout appairage. Si la valeur d'env est absente ou correspond à une valeur d'exemple connue (`change-me`, `''`), génère une valeur aléatoire (`randomBytes(32).toString('hex')`) et la persiste ; retourne `{jwtGenerated}`. **Plus de génération de mot de passe technicien** (`TECH_PASSWORD` retiré, PROJET.md §11.30) — la fenêtre pré-PIN est couverte par la session émise directement dans `POST /sync/onboarding/pair`, cf. `routes/sync.js` plus bas. |
...
| `middleware/auth.js` | `makeAuthRouter` (login → JWT), `requireRole(role)` factory → `requireAdmin`/`requireTech`. **Phase C** — ni `admin_borne` ni `tech_borne` n'ont de compte nominatif : `{ pin }` comparé à `event_meta.tech_pin` (essayé en premier, rôle le plus élevé) puis `event_meta.admin_pin` de l'événement actif ; 401 si `pin` absent du corps ou si aucun des deux ne correspond. **Plus de fallback par mot de passe** (`TECH_PASSWORD` retiré, PROJET.md §11.30) — la fenêtre avant le premier PIN (juste après appairage, avant le premier pull réussi) est couverte par `POST /sync/onboarding/pair` (`routes/sync.js`), qui émet directement une session `tech_borne` dès que le token borne est validé par le Hub. ⚠️ **Corollaire** : un événement actif dépourvu de `tech_pin` en `event_meta` (pullé d'un Hub antérieur à Phase C) rend la console `/borne` inaccessible jusqu'au prochain pull — c'est le heartbeat qui répare. |
| `initLog.js` | **Phase C** — journal en mémoire (~100 entrées, `logInit(level, message)`/`getInitLog()`) alimenté par `index.js` (démarrage), `pull.js` (pulls initiaux) et `heartbeat.js` (battements). Non persisté (redémarrage = journal vide). Exposé sans auth via `GET /api/sync/pairing-status` **uniquement tant que la borne n'est pas appairée** (voir `routes/sync.js`). |
...
| `routes/sync.js` | endpoints déclenchant `pull.js`/`push.js`, status de progression. **Phase B** — `GET /sync/status` expose `tokenKind: 'borne'\|'event'` (identité machine persistante vs token d'événement preview/legacy) en plus du token masqué ; `POST /sync/token` (authentifiée, rotation) délègue à `applyNewToken(trimmed)`, factorisée avec la route d'appairage ci-dessous — détection du type de token (borne physique vs preview/événement) en interrogeant `GET /api/sync/borne/events` côté Hub (400 = token d'événement), persistance via `setSetting('borne_token', …)`, `pullMyEvents()`/`pullMyEvent()` selon le type, et **`startHeartbeat(dataDir)`** (no-op si déjà démarré) côté borne physique. **Phase C** — `GET /sync/pairing-status` : **seule route sans `requireTech`** de ce fichier avec `POST /sync/onboarding/pair` ci-dessous, volontairement (onboarding pré-appairage), et **rate-limitée** (60/min/IP, `skip` via `cfg.skipRateLimits`). Réponse **à deux formes** : borne non appairée (`hasToken=false`) → `{hasToken, hubUrl, hasActiveEvent:false, lastPull:null, logs}` (l'écran d'onboarding a besoin de la progression) ; borne appairée → **`{hasToken:true, hasActiveEvent}` et rien d'autre** — `hubUrl` (topologie interne, ex. `hub-backend:3001`), `lastPull` et le journal d'init ne fuitent jamais sur une borne en service, a fortiori une preview Internet-facing. Jamais de token en clair dans aucun des deux cas. ⚠️ **`hasToken` se calcule sur l'identité PERSISTÉE** (`getSetting('borne_token')`), pas sur `config.borneToken` — ce dernier peut porter le candidat d'un appairage **raté** laissé en mémoire, et basculer `hasToken` à `true` empêcherait l'`OnboardingScreen` de se réafficher au rechargement (`sawUnpairedRef` ne se latche qu'une fois) alors qu'aucun PIN n'existe encore : console verrouillée jusqu'au redémarrage du conteneur. Le `boxToken` (preview/legacy) reste lu **en mémoire** dans ce calcul (`cfg.boxToken || config.boxToken`) — il n'est jamais persisté, le provisioner le réinjecte par l'env à chaque (re)création du conteneur. ⚠️ **`hasToken` et le verrou de `POST /sync/onboarding/pair` ne calculent donc PAS le même prédicat « appairée »** (le verrou ignore `boxToken`, `hasToken` ignore `paired_at`/`local_events`) — les deux divergences résiduelles sont détaillées dans le piège « routes Borne sans auth » plus bas.
**`POST /sync/onboarding/pair`** (Phase C, sans auth) : appairage initial depuis l'écran d'onboarding — **404 en mode preview** (`isPreviewMode(cfg)` : surface Internet-facing, pas de formulaire « appairez-moi à n'importe quel Hub »), puis **403** si la borne a déjà été appairée. Le verrou est un **OU de trois signaux persistés**, jamais `getLastPull()` (singleton en mémoire remis à zéro par chaque redémarrage, alors que la borne est censée tourner offline pendant l'événement, §1) : (a) `borne_settings.paired_at !== null` — posé au premier pull réussi par `sync/pull.js` ; (b) **au moins une ligne dans `local_events`** — signal de fait, couvre une borne mise à niveau depuis une version antérieure à `paired_at` (ses événements déjà pullés valent preuve, sans attendre un pull de plus) ; (c) **`getSetting('borne_token') !== null`** — borne seedée par `BORNE_TOKEN` dans le `.env`, que `resolveBorneIdentity()` persiste au boot **sans** round-trip Hub ni `paired_at` : sans ce signal, un tiers sur le LAN pourrait réappairer une machine que `pairing-status` annonce pourtant déjà `hasToken:true` (donc sans formulaire proposé). Tant qu'aucun des trois signaux n'est posé, un nouvel essai (token corrigé) est accepté — PROJET.md §11.30 : un token mal recopié **par ce formulaire** ne verrouille jamais la borne (un `BORNE_TOKEN` mal recopié dans le `.env`, lui, ferme le verrou dès le boot via (c) — contrepartie assumée de (c), cf. piège plus bas). Le signal (b) n'est plus piégeux : `pullEvent()` n'écrit la ligne `local_events` qu'**en toute dernière étape**, après les assets de design (cf. `sync/pull.js`). **`applyNewToken` retourne `{isBorneToken, pull:{ok, pulled?, error?}, hasActiveEvent}`** (pas juste un booléen) — la route y ajoute `token` : un JWT `{roles:['tech_borne']}` signé avec `config.jwtSecret` (le singleton **live** ; depuis le passage de `routerCfg` à `Object.create(cfg)`, `cfg.jwtSecret` y retomberait aussi, mais l'import direct reste le chemin explicite) **si et seulement si `pull.ok`** — la preuve d'autorisation est le round-trip Hub abouti lui-même, plus de `TECH_PASSWORD` à connaître en plus (§11.30). **Persistance différée à la preuve** : `borne_token` (dans `applyNewToken`) et `hub_url` (dans la route) ne sont écrits en base **que si `pull.ok`** ; sinon l'état d'avant est restauré en mémoire, **sans condition** dans les deux branches (borne *et* événement) — y compris quand cette valeur précédente est vide, puisque ni le verrou ni `hasToken` ne lisent plus un candidat de borne raté. ⚠️ `hubUrl` vient du corps d'une requête **non authentifiée** et n'est validé ni en schéma ni en hôte avant d'être passé à `hubFetch` : la route fait donc émettre à la borne des requêtes sortantes vers une URL arbitraire (SSRF depuis le LAN), et renvoie le statut/message d'erreur distant dans `pull.error`. Réponse finale `{ok:true, tokenKind, pull, hasActiveEvent, token}` — jamais un `{ok:true}` de façade : le pull peut échouer (Hub injoignable) alors que la requête répond 200, auquel cas `token` reste `null`, **rien n'est persisté ni verrouillé** (prochain essai autorisé) et **aucun heartbeat n'est démarré** (`startHeartbeat` n'est appelé que dans la branche `pull.ok`) — il n'y a donc **pas** de re-essai automatique en tâche de fond, c'est l'écran d'onboarding qui resoumet. `POST /sync/token` (rotation, authentifiée) partage le même helper mais ignore ce détail dans sa réponse (`{ok:true}` seul, IdentityPanel n'en a pas besoin — l'appelant est déjà authentifié). |
...
| `pull.js` | `pullMyEvent` (preview/token=événement — récupère l'unique événement du token, **active explicitement** après `pullEvent` pour préserver le comportement historique) → `pullEvent` (écrit questions/meta dans db.sqlite, DELETE+INSERT ; **depuis Phase B, ne pose plus `active=1` lui-même** — sorti pour que puller un second événement ne bascule jamais le kiosque hors d'un premier `live`). Depuis Phase C, le bundle ne transporte plus aucun compte nominatif (`event_users` de la Borne reste **toujours vide** — `admin_borne`/`tech_borne` passés au PIN partagé, `event_meta.admin_pin`/`tech_pin`) ; **`DELETE FROM event_users` est exécuté à chaque pull** — purge one-shot du matériel d'auth (email + hash argon2) laissé par une version antérieure à Phase C sur une borne mise à niveau ; écrit `requires_login` dans `event_meta` (§11.24). Statut local vérifié **au moment d'appliquer** (§11.10). En preview : purge les anciens événements preview. ⚠️ **Ordre des étapes (réordonné Phase C)** : `insertEvent()` + `UPDATE … pulled_at` sont exécutés en **toute dernière étape**, après les assets de design — la ligne `local_events` sert de **preuve** qu'un pull a abouti (signal (b) du verrou de `POST /sync/onboarding/pair`), elle ne doit donc jamais exister sur un pull partiel. Contreparties : le `existing` lu à l'étape 2 est utilisé à l'étape 6 (fenêtre élargie à la durée du téléchargement des assets — deux pulls concurrents du même événement se marcheraient dessus sur la clé primaire), et un échec après l'étape 3 laisse un `events/<id>/` (db.sqlite + design) **sans ligne registre**, donc invisible de `POST /sync/purge/:eventId` (dossier orphelin inerte, sans PII : les sessions/vidéos n'existent pas encore à ce stade). `_setLastPull()` (appelé par `pullMyEvent`/`pullMyEvents`) pose aussi `borne_settings.paired_at`, **une seule fois** dans la vie de la borne. **Étape 5 — `pullDesignAssets`** (§9bis) : `events/<id>/design/` est **rasé puis reconstruit à chaque pull** (un asset d'un design retiré côté Hub ne survit pas), chaque fichier de `bundle.design_assets` est téléchargé (`hubFetchBuffer`) puis **vérifié par sha256 ; un mismatch fait ÉCHOUER le pull** (throw) après avoir supprimé le dossier (invariant §11.27). Le filename de chaque asset (venu du réseau) est **validé par `isValidAssetFilename` AVANT** tout `join`/`writeFileSync` — un `../db.sqlite` fait échouer le pull, jamais une écriture arbitraire. Un bundle sans `design_assets` (Hub non migré) est toléré (`?? []`). `event_meta.design` est écrit à l'étape 5, **avant** le téléchargement, mais un échec (checksum OU filename invalide) déclenche `dropDesignMeta(eventDir)` avant de propager l'erreur → la clé `design` est retirée, le kiosque retombe sur le thème figé au lieu de servir des images 404 (pas d'état partiel). **Phase B — `pullMyEvents(dataDir)`** (borne physique, plusieurs événements assignés) : `GET /api/sync/borne/events` puis boucle `pullEvent()` par événement, chacun protégé individuellement par §11.10 (un événement `live`/`closed` local est *skip*, pas d'erreur globale). N'active **jamais** un événement au détriment d'un autre — seule exception : aucun actif ET un seul pull réussi → activation bootstrap (évite qu'une borne neuve reste bloquée sur l'écran de chargement) ; dès que ≥ 2 événements coexistent, l'activation redevient un choix explicite (console `/borne` ou commande Hub `activate_event`). 400 Hub (route utilisée avec un token preview) → `{pulled:0, results:[]}`, pas une erreur. |
...
- **Phase B — deux consoles, deux sessions localStorage, deux auth distinctes** (`App.jsx` route sur `window.location.pathname`, toujours pas de react-router) :
  - **Racine (`isGuestRoot`, ni `/borne` ni `/admin`)** : **Phase C** — tant que la borne n'est pas
    appairée (`useOnboardingGate`, cf. ci-dessous), `App.jsx` rend `<OnboardingScreen/>` directement,
    **avant** le wrapper `.kapsule-guest` — sinon les classes `.kapsule-guest .screen`/`.text--muted`
    (plus spécifiques, dupliquées dans `@kapsule/guest-ui/guest.css` pour le parcours invité, cf. ligne
    ~278) l'emporteraient sur les siennes et casseraient sa mise en page. Une fois appairée (ou
    confirmée via `onDone`), retombe sur `<GuestPage>` normal, dans son wrapper `.kapsule-guest`.
    `useOnboardingGate(isGuestRoot)` est monté **inconditionnellement** sur la racine, donc aussi sur
    chaque onglet visiteur d'une borne d'essai Internet-facing. Son `setInterval` de 4 s sur
    `GET /api/sync/pairing-status` **s'arrête dès que `stopWhen(status)` est vrai** — 2ᵉ paramètre de
    `usePairingStatus(active, stopWhen)`, défaut `DEFAULT_STOP_WHEN = (s) => s.hasToken`
    (`clearInterval` dans le `.then`) : sinon 15 req/min/onglet contre un limiteur de 60/min/IP,
    4 onglets suffisaient à déclencher des 429 pour une information figée. Le prédicat est
    **paramétrable par appelant** précisément parce que la condition d'arrêt n'est pas la même
    partout : `GuestPage` réutilise le hook pour son `NotConfiguredScreen` (cf. plus bas) et lui
    passe `stopAtActiveEvent = (s) => s.hasActiveEvent` (constante **module-level** — `stopWhen`
    n'est volontairement pas dans les deps du `useEffect`, une arrow inline relancerait le polling à
    chaque rendu). Avec le défaut, cet écran ne se serait jamais réparé seul (`hasToken` est déjà
    vrai au premier tick sur une borne appairée).
    ⚠️ **Angle mort restant** : le gate se déclenche sur `hasToken`, pas sur « appairée ». Une borne
    réelle appairée avec un **token d'événement** (branche `tokenKind:'event'`) ne persiste rien —
    après un redémarrage `hasToken` repasse à `false`, l'`OnboardingScreen` se remet devant le
    kiosque **et** devant `/borne`, alors que `POST /sync/onboarding/pair` répond 403 (verrou fermé
    par `paired_at`/`local_events`) : plus aucun écran atteignable une fois le `tech_token` de 24 h
    expiré.
  ...
  - **Phase C — écran d'onboarding pré-appairage** (`OnboardingScreen.jsx` + les hooks `usePairingStatus`
    et `useOnboardingGate` exportés par le même fichier) : tant que la borne n'est pas appairée,
    `GET /api/sync/pairing-status` (**sans auth**, toutes les 4 s) est interrogée et `null` est rendu au
    tout premier chargement pour éviter un flash du mauvais écran. **`OnboardingScreen` est rendu
    identiquement à deux endroits** — `/borne` (`BornePage.jsx`) et la **racine** (`App.jsx`, kiosque
    invité par défaut, cf. ci-dessous) — via le hook partagé `useOnboardingGate(active)`, pour ne pas
    dupliquer la logique deux fois.
    **Tracker en 4 étapes** (`computeSteps()` dans `OnboardingScreen.jsx`, jamais côté serveur — dérivé
    localement de `status`, de l'état du formulaire et de `pairResult`) : *Serveur borne* (`status._error`
    → sondage injoignable), *Hub* (`status.hubUrl` préconfiguré, ou validé une fois `pairResult` reçu),
    *Token* (à saisir → envoi en cours → accepté, ou erreur de soumission), *Événement* (en attente du
    token → résultat concret du pull une fois connu). Affiché en permanence, avant ET après l'appairage
    — c'est la seule route qui répond sans auth, donc le seul moyen de vérifier que le backend répond
    vraiment (étape 1) sans se fier à un silence qui pourrait aussi bien être un conteneur down qu'un
    simple 429 du rate-limiter.
    Le **formulaire d'appairage** (token + URL Hub, cette dernière masquée si déjà préconfigurée) poste
    sur `POST /api/sync/onboarding/pair` (`api.onboardingPair`, sans auth) — alternative à éditer
    `BORNE_TOKEN` dans le `.env` et redémarrer, toujours possible en repli.
    La réponse de `POST .../pair` (`{ok, tokenKind, pull, hasActiveEvent, token}`, cf. `routes/sync.js`)
    est gardée en état local (`pairResult`) et rendue **telle quelle**, jamais reformulée en simple
    succès : `pull.ok` faux → le formulaire **reste affiché** (pas remplacé par un écran terminal) avec
    le message d'erreur, pour resoumettre un token corrigé sans recharger la page (§11.30 PROJET.md :
    rien n'est verrouillé tant qu'aucun pull n'a abouti) ; `pull.ok` vrai → `result.token` (JWT
    `tech_borne`, émis par le serveur puisque le round-trip Hub a validé ce token) est **sauvegardé
    immédiatement** via `saveTechToken` — plus de `TECH_PASSWORD` à taper en plus (retiré, §11.30) — et
    le panneau « Appairage terminé » s'affiche (événement chargé, ou invite à en assigner un). Un
    bouton **« Continuer »** (`onDone`) est le seul déclencheur du passage à l'écran suivant, et
    n'apparaît que si `pull.ok` — **pas** `hasToken` seul : `useOnboardingGate` retient
    (`sawUnpairedRef`, un `useRef` — mutation visible dans le même rendu, pas au suivant comme un
    effect, pour ne jamais flasher l'écran suivant avant `OnboardingScreen` sur une borne vraiment
    neuve) avoir vu la borne non-appairée pendant cette session, et dans ce cas reste dessus **même
    après que `hasToken` passe à `true`**, jusqu'au clic. Une borne déjà appairée quand la page se
    charge (cas quotidien) ne passe jamais par cet état : va droit à l'écran suivant, comme avant.
    Côté `/borne`, `BornePage` ré-vérifie `isTechAuthenticated()` dans son `onDone` — le token sauvegardé
    pendant l'appairage suffit à sauter `AdminLogin` entièrement ; sans lui (pull.ok faux, jamais
    atteint dans ce cas puisque le bouton n'apparaît pas), on tombe sur `AdminLogin` en **mode PIN
    unique** (plus de mode `'password'`, §11.30). Côté racine, `onDone` retombe simplement sur le
    kiosque invité normal (`GuestPage`) — le token sauvegardé profite silencieusement d'une visite
    ultérieure sur `/borne`.
    ⚠️ **Cas limite restant** : si le tout premier appel à `pairing-status` échoue (réseau, 429 du
    rate-limiter avec plusieurs onglets ouverts), `usePairingStatus` **ne reste pas à `null`** : après
    le premier échec il pose un statut dégradé `{ hasToken:false, _error:true, … }` (l'étape *Serveur
    borne* du tracker passe en erreur, jamais d'écran blanc), et le polling toutes les 4 s se rétablit
    seul dès la première réponse.
...
(miniature/durée/dimensions, 1-1 avec videos), `event_users` (email/hash/roles — **table morte depuis Phase C** : plus jamais peuplée, le
bundle ne transporte plus de compte ; conservée pour ne pas casser le schéma des bases existantes),
...
`GET /api/sync/events/:id/bundle` (questions + meta + `design_assets`, passe `ready`→`loaded` — plus aucun compte nominatif depuis Phase C, `admin_pin`/`tech_pin` voyagent dans `meta`) →
...
- *Borne* : JWT `{roles}` (admin_borne/tech_borne via PIN partagé, sans email depuis Phase C ;
  general = `{email, roles, event_id}` via preview/token), 24 h. `tech_borne` ⊇ `admin_borne` (§11.19).
  Plus de fallback par mot de passe (`TECH_PASSWORD` retiré, §11.30) — `POST /api/admin/login`
  n'accepte que `{ pin }`. La fenêtre pré-PIN est couverte par la session émise directement par
  `POST /sync/onboarding/pair` (cf. §6bis/§205 ci-dessus).
  ⚠️ **Historique (corrigé)** : l'auth wall preview (`general`) était câblée par erreur côté front
  sur `api.login` → `POST /api/admin/login` (`GuestPage.jsx`) — mort depuis la Phase C (plus de
  compte nominatif), le chemin valide est `POST /api/preview/login` (`routes/sessions.js`), réparé
  en Phase C.2 ; `api.login` lui-même a depuis été supprimé (client.js) avec le retrait de
  `TECH_PASSWORD`, n'ayant plus aucun appelant.
...
- **Durcissement exposition publique** (Hub + Borne preview Internet-facing) : `express.json({ limit: '1mb' })`, `trust proxy` **= le nombre réel de hops** (Hub : figé à `1` dans `apps/hub/server/src/index.js`, sa topologie n'ayant qu'un edge-nginx devant lui ; Borne : `cfg.trustProxyHops`, issu de `resolveTrustProxyHops(env)` dans `config.js` — `TRUST_PROXY_HOPS` sinon 1 borne réelle / 2 preview, §11.31 PROJET.md ; une valeur trop petite, ou `NaN` sur une variable d'env vide, fait retomber `req.ip` sur l'IP d'un nginx interne et met **tous** les rate-limiters dans un seul seau), `express-rate-limit` (Hub login 10/15min ; Borne login 10, sessions 20, uploads 50, `pairing-status` 60/min — `skip` via `cfg.skipRateLimits` en test), en-têtes de sécurité (HSTS/X-Frame/nosniff/CSP) posés au niveau **edge** et borne-nginx, TLS terminé par le service `edge`. Côté **Borne**, `resolveJwtSecret()` (Phase C) élimine la situation à la racine — `JWT_SECRET` absent ou égal à une valeur d'exemple (`''`/`change-me`) est remplacé par une valeur générée **et persistée** (`borne_settings.jwt_secret`) avant même que `validateConfig` s'exécute ; celle-ci reste appelée juste après comme filet de sécurité (ne devrait plus jamais se déclencher en pratique). `TECH_PASSWORD` a été retiré (§11.30 PROJET.md) : plus rien à générer ni à valider de ce côté. ⚠️ **La valeur persistée prime ensuite sur l'env, sans échappatoire** : une fois `jwt_secret` en base, réécrire `JWT_SECRET` dans le `.env` (ou dans l'env injecté par le provisioner) n'a plus **aucun** effet. Sur une borne **preview**, le provisioner injecte le `JWT_SECRET` **du Hub** (partage assumé — le Hub signe les JWT `general` de `POST /events/:id/preview/token` que la borne vérifie) : ce secret est donc figé sur disque dans `preview-data-<slug>/registry.sqlite` au premier boot, et une rotation côté Hub ne le remplace pas (liens `preview/token` neufs rejetés, anciens toujours acceptés). Côté **Hub**, aucun équivalent : `apps/hub/server/src/config.js` garde `jwtSecret: env.JWT_SECRET ?? 'change-me'` et `validateConfig` ne lève **que si `NODE_ENV === 'production'`** — or ni `apps/hub/server/Dockerfile` ni `docker-compose.hub.yml` ne posent `NODE_ENV` (contrairement à `docker-compose.borne.yml`), donc le garde-fou est **inerte en déploiement réel** et le défaut `${JWT_SECRET:-change-me}` du compose peut démarrer un Hub Internet-facing avec un secret public.
- **`routerCfg` (Borne, `index.js`) délègue à `config` par prototype — ne plus le construire par spread** : `const routerCfg = Object.assign(Object.create(cfg), { requireAdmin, requireTech })`. La forme précédente (`{ ...cfg, … }`) copiait les valeurs au moment de `createApp()`, donc **avant** `resolveJwtSecret()`/`resolveBorneIdentity()` (appelés par le point d'entrée réel juste après) : `routes/sessions.js`, qui lit `cfg.jwtSecret` pour l'auth wall preview (§11.24), signait et vérifiait avec l'**ancienne** valeur pendant que le reste du process utilisait la nouvelle. Avec `Object.create`, `routerCfg` n'a que `requireAdmin`/`requireTech` en propriété **propre** ; toute autre lecture (`jwtSecret`, `hubUrl`, `boxToken`, `previewMode`, `skipRateLimits`, `maxDataBytes`…) retombe sur `cfg` — donc, en production où `cfg === config`, sur la valeur **live** du singleton. ⚠️ **Le corollaire à ne pas casser** : une **écriture** via `cfg.<x> = …` crée une propriété propre qui masque définitivement le prototype. `routes/sync.js` (`applyNewToken`, route d'appairage) écrit `cfg.borneToken`/`cfg.boxToken`/`cfg.hubUrl` **en miroir** de `config.*` — c'est ce miroir qui maintient la cohérence, pas la délégation. Toute nouvelle écriture doit soit rester en miroir, soit (mieux) ne toucher que `config.*`. Aucun code ne fait de spread / `Object.keys` / `JSON.stringify` de `routerCfg` (ce qui ne verrait que les deux propriétés propres) — à re-vérifier avant d'en ajouter un.
- **Deux routes Borne sans aucune auth** (`routes/sync.js`, Phase C) : `GET /sync/pairing-status` et
  `POST /sync/onboarding/pair`, toutes deux derrière le même limiteur 60/min/IP. La seconde **écrit**
  (`setSetting('hub_url')`, `setSetting('borne_token')`) et **émet un JWT `tech_borne`** quand le pull
  réussit ; elle est **404 en mode preview** et son verrou est un OU de **trois** signaux
  **persistés** : (a) `borne_settings.paired_at` (posé au premier pull réussi), (b) au moins une
  ligne `local_events` (borne mise à niveau depuis une version antérieure à `paired_at`, dont les
  événements déjà pullés valent preuve), (c) `borne_settings.borne_token` non nul (borne seedée par
  `BORNE_TOKEN` en `.env`, persisté au boot par `resolveBorneIdentity()` sans round-trip Hub).
  Persistés, donc survivant à un redémarrage — une borne offline pendant l'événement (cas nominal
  §1) ne rouvre plus cette route. Le signal (b) n'est plus prématuré : `pullEvent()` écrit
  `local_events` en toute dernière étape.
  ⚠️ **Angles restants — tous sur `boxToken`, jamais persisté**, alors que le verrou ne connaît que
  des signaux persistés et que `hasToken` (`pairing-status`) le lit, lui, en mémoire :
  (1) *verrou trop étroit* — une borne **réelle** seedée avec `BOX_TOKEN` (configuration découragée,
  `.env.example-rasp`) dont aucun pull n'a jamais abouti répond `hasToken:true` (donc l'UI ne propose
  aucun formulaire) mais laisse la route **ouverte** sur le LAN : un tiers peut réappairer la machine
  sur son propre Hub et recevoir un JWT `tech_borne` en retour. C'est le symétrique exact du signal
  (c), non ajouté pour `boxToken` ;
  (2) *`hasToken` trop étroit* — une borne réelle appairée avec un **token d'événement** via ce
  formulaire ne persiste rien : au redémarrage `hasToken` repasse à `false` (l'`OnboardingScreen`
  reprend le dessus sur le kiosque et sur `/borne`) alors que le verrou, lui, est fermé (403). Voir
  l'angle mort du gate front, §3 Frontend borne.
  La correction de fond est un **prédicat « appairée » unique**, partagé par `pairing-status` et par
  le verrou (et une persistance du token d'événement, ou son refus explicite depuis l'onboarding).
  ⚠️ Enfin, `hubUrl` du corps n'est **pas validé** (schéma/hôte) avant d'être passé à `hubFetch` :
  SSRF non authentifiée depuis le LAN, dont le statut et le message d'erreur distants reviennent dans
  `pull.error`.
- **`requireOwner` (Hub) ignore le contenu de `roles`** : toute ligne `event_users` — y compris
  `roles: ['general']`, seul rôle encore assignable depuis Phase C — donne un accès **propriétaire**
  complet à l'événement (config, galerie, et depuis Phase C `meta.admin_pin`/`meta.tech_pin` via
  `GET /api/events/:id`). Assigner un « invité » de l'auth wall preview revient donc à lui livrer le
  PIN technicien de la borne physique. À traiter avant d'ouvrir l'assignation à des tiers.
