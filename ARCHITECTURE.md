...
| `/api/admin` | `routes/admin.js` | `requireUser` + **`requireSuperuser`** | gestion comptes clients, box_tokens (génération/liste/révocation), event_users (assignation), overview dashboard. **`POST /users/:id/send-registration`** : génère un token + URL (`buildRegistrationUrl`) **et** envoie l'email (envoi SYNCHRONE, try/catch englobant, journalisé `email_logs` en `sent`/`skipped`/`failed`). Renvoie **toujours** `{ registration_url, email_sent }` — un échec SMTP ne fait jamais échouer la requête (fallback lien copiable). |
